...
function body(s,internal=false,exportId='Preview'){
 ...
 if(g.grams)rows.unshift(['Gold holdings',g.batches.length+' proposed / booked transactions',C.money(g.invested),C.money(g.current),'Price date: '+g.priceDate]);
 html+=panel('Holdings',table(['Product','Status','Invested','Current value','Maturity / valuation'],rows));
 if(p.holdings.some(h=>h.note||h.yield||h.start))html+=panel('Product details',table(['Product','Start','Yield / coupon','Notes'],p.holdings.map(h=>[h.name,h.start||'Not entered',C.pct(C.num(h.yield)),h.note||''])));
 if(g.batches.length)html+=panel('Gold transactions',`<p class="small">Purchase prices include the premium and are expressed in AED per gram. Valuation date: ${E(g.priceDate)}.</p>`+table(['Date / reference','Status','Quantity (g)','Purchase price AED/g','Purchase amount AED'],g.batches.map(b=>[b.date+(b.reference?' · '+b.reference:''),b.status,C.fmt(b.grams),C.fmt(b.allInAEDg,4),C.fmt(b.clientTotalAED)])));
 if(p.client.notes)html+=panel('Client notes',`<div dir="auto" class="report-note">${E(p.client.notes)}</div>`);
 if(internal){
  const x=C.goldSummary(s);
  html+=panel('Internal · Revenue and pricing',`<div class="kpis">${kpi('Client value',C.money(x.client))}${kpi('Market cost',C.money(x.spot))}${kpi('Gross revenue',C.money(x.revenue))}${kpi('Markup on market cost',C.pct(x.margin))}${kpi('Revenue per gram',C.fmt(x.grams?x.revenue/x.grams:NaN,4),'AED / g')}</div><div dir="auto" class="report-note">${E(s.client.notesInternal)}</div>`+table(['Reference / date','Market cost AED','Revenue AED','Premium','Internal note'],x.valid.map(b=>[b.reference||b.date,C.fmt(b.spotTotalAED),C.fmt(b.revenueAED),C.pct(b.premium),b.internalNote||'']))+`<h3>Report history</h3>`+table(['Document reference','Audience / format','Prepared at (UTC)'],s.reportHistory.slice(0,12).map(r=>[r.id,r.type+' / '+r.format,r.at]))+`<h3>Activity log</h3>`+table(['Time (UTC)','Action','Detail'],s.auditLog.slice(0,12).map(r=>[r.at,r.action,r.detail])));
 }
 html+=`<footer class="report-foot"><div>RAMY · ${internal?'Confidential · Internal':'Portfolio report'}<br>${E(p.report.date)}<br>Prepared using the recorded portfolio information.</div><div class="signature">${C.safeImage(a.signatureImage)?`<img alt="Advisor signature" src="${C.safeImage(a.signatureImage)}">`:''}<b dir="auto">${E(a.name||'Ramy')}</b><span dir="auto">${E([a.title,a.company,a.phone,a.email].filter(Boolean).join(' · '))}</span></div></footer></article>`;
 return html;
}
