...
const artwork=n=>globalThis.RAMY_ARTWORK?.[n]?`<img class="section-art" src="${RAMY_ARTWORK[n]}" alt="" aria-hidden="true" width="96" height="96">`:'';
...
const footer=()=>`<footer class="app-footer"><span>Client portfolio</span><b>RAMY</b></footer>`;
...
function context(){const mode=state.ui.mode[0].toUpperCase()+state.ui.mode.slice(1);$('clientContext').textContent=(state.client.name||'No client selected')+' · '+mode;$('clientContext').title=[state.client.name,state.client.reference,mode].filter(Boolean).join(' · ');document.documentElement.dataset.theme=state.ui.theme;$('brandLogo').src=C.safeImage(state.advisor.logoImage)||RAMY_LOGO;document.querySelector('meta[name="theme-color"]').content=state.ui.theme==='blue'?'#eaf0f6':'#f4eee4';}
...
function nav(){
 const labels=isClient()?{results:'Overview',reports:'Reports'}:{client:'Client',gold:'Gold',portfolio:'Holdings',results:'Overview',reports:'Reports'};
...
function render(next=screen,focus=false){
 ...
 if(screen==='gold'||screen==='portfolio'){
  ...
  const tabs=document.createElement('div');tabs.className='entry-tabs';tabs.innerHTML=btn(gold?'Transaction details':'Investment details',action,tab==='entry'?'primary':'','data-tab="entry" aria-pressed="'+(tab==='entry')+'"')+btn((gold?'Transactions':'Holdings')+' ('+count+')',action,tab==='records'?'primary':'','data-tab="records" aria-pressed="'+(tab==='records')+'"');
...
function welcomeView(){return `<section class="welcome"><div class="welcome-panel panel"><div class="welcome-heading"><div><h1>Portfolio workspace</h1><p>Client records, investment calculations and reporting.</p></div>${artwork('portfolio')}</div><div class="actions">${btn('Open client profile','start','primary')}${btn('Import client record','import')}</div><div class="workspace-grid">${[['gold','Gold calculator','Purchase pricing'],['portfolio','Investment holdings','Portfolio records'],['results','Portfolio overview','Valuation and performance'],['reports','Reports','Client and internal documents']].map(([section,title,description])=>`<button class="workspace-tile" type="button" data-action="nav" data-screen="${section}">${artwork(section)}<span><strong>${title}</strong><small>${description}</small></span>${icon('next')}</button>`).join('')}</div><div class="helper">${state.client.name?'Last opened: '+E(state.client.name):'No client selected.'}</div><details class="disclosure"><summary>Privacy and local storage</summary><p class="small">Records are stored in this browser on this device. Keep an editable HTML or JSON backup before clearing browser data. Display modes do not provide password protection. Share client reports only; editable records contain internal information.</p></details></div></section>`;}
function clientView(){return intro('client','Client profile','Client information and reporting preferences.')+`<section class="panel">${head('Client details','Required fields are marked with an asterisk.')}<div class="grid">${field('client.name','Client name *')}${field('client.reference','Client reference')}${field('client.segment','Client category',{options:{HNWI:'HNWI','Private Client':'Private client',Corporate:'Corporate','Family Office':'Family office',Other:'Other'}})}${field('client.preferredLanguage','Preferred language',{options:{English:'English',Arabic:'Arabic',Bilingual:'Bilingual'},hint:''})}</div><details class="disclosure"><summary>Notes and confidentiality</summary><div class="grid">${field('client.notesClient','Client report notes',{type:'textarea'})}${field('client.notesInternal','Internal notes',{type:'textarea',hint:'Not included in client reports.'})}</div></details><div class="actions">${btn('Gold calculator '+icon('next'),'client-next','primary next')}${btn('Investment holdings','client-portfolio')}</div></section><section class="panel">${head('Client records','Current changes are saved before another record is opened.')}<div class="actions">${btn('Saved clients','records')}${btn('New client','new-client')}${btn('Import HTML / JSON','import')}</div><p class="small" style="margin:15px 0 0">Records are stored on this device. Keep a separate backup.</p></section>`;}
function goldView(){return intro('gold','Gold calculator','Purchase pricing and transaction records.',state.gold.draft.id?'Edit transaction':'New transaction')+`<section class="panel">${head('Pricing details',state.gold.draft.id?'Original transaction prices are retained in AED per gram.':'Enter the quantity and any two of spot price, purchase price and premium.')}<div class="grid">${field('gold.draft.quantity','Quantity *',{type:'decimal'})}${field('gold.draft.quantityUnit','Quantity unit',{options:{g:'Grams (g)',kg:'Kilograms (kg)',oz:'Troy ounces (oz)'}})}${field('gold.draft.currency','Price currency',{options:{AED:'AED',USD:'USD'}})}${field('gold.draft.priceUnit','Price unit',{options:{per_g:'Per gram',per_kg:'Per kilogram',per_oz:'Per troy ounce'}})}${field('gold.draft.spot','Spot price',{type:'decimal'})}${field('gold.draft.allIn','Purchase price (all-in)',{type:'decimal'})}${field('gold.draft.premium','Premium (%)',{type:'decimal',hint:'Leave blank to calculate the premium from the two prices.'})}</div><div id="goldDraftPreview" class="helper" aria-live="polite"></div><details class="disclosure" ${state.gold.draft.id?'open':''}><summary>Transaction details and notes</summary><div class="grid">${field('gold.draft.date','Transaction date',{type:'date'})}${field('gold.draft.reference','Transaction reference')}${field('gold.draft.schedule','Schedule',{options:{one_off:'One-off',monthly:'Monthly',weekly:'Weekly',flexible:'Flexible / manual',existing:'Existing holding'}})}${field('gold.draft.status','Transaction status',{options:{proposal:'Proposal',booked:'Booked',cancelled:'Cancelled'}})}${field('gold.draft.clientAmount','Target amount — reference only',{type:'decimal',hint:'For reference only. This amount does not determine the quantity.'})}${field('gold.draft.internalNote','Internal transaction note',{type:'textarea'})}</div></details><div id="goldError" class="error-box" role="alert"></div><div class="actions">${btn((state.gold.draft.id?'Save transaction':'Add transaction'),'add-gold','primary')}${btn(state.gold.draft.id?'Cancel edit':'Clear inputs','clear-gold')}${btn('Portfolio overview '+icon('next'),'nav','subtle','data-screen="results"')}</div></section><section class="panel">${head('Gold transactions',state.gold.batches.length+' recorded transactions')}<div class="record-list">${state.gold.batches.map(b=>`<article class="record-card"><h3 dir="auto">${E(b.reference||'Gold purchase')}</h3><div class="record-meta">${E(b.date)} · ${E(statusLabel(b.status))} · ${E(scheduleLabel(b.schedule))}</div><div class="record-values"><div><small>Quantity</small><strong class="num">${C.fmt(b.grams,2)} g</strong></div><div><small>Client total</small><strong class="num">${C.money(b.clientTotalAED)}</strong></div><div><small>All-in price</small><strong class="num">${C.fmt(b.allInAEDg,4)} AED/g</strong></div><div><small>Premium</small><strong class="num">${C.pct(b.premium)}</strong></div></div><div class="record-actions">${btn('Edit','edit-gold','','data-id="'+E(b.id)+'"')}${btn('Delete','delete-gold','danger','data-id="'+E(b.id)+'"')}</div></article>`).join('')||'<div class="empty">No gold transactions recorded.</div>'}</div>${state.gold.batches.length?`<details class="disclosure"><summary>Manage transactions</summary>${btn('Delete all gold transactions','clear-batches','danger')}</details>`:''}</section>`;}
function goldDraftPreview(){
 const el=$('goldDraftPreview');if(!el)return;if(!['quantity','spot','allIn','premium'].some(k=>String(state.gold.draft[k]).trim())){el.textContent='The purchase amount will appear here once the pricing details are complete.';return;}try{const b=C.batchFromDraft(state.gold.draft,state.fx);el.innerHTML=`<b>Purchase amount · not yet recorded</b><br><span class="num">${C.fmt(b.grams,2)} g × ${C.fmt(b.allInAEDg,4)} AED/g = ${C.money(b.clientTotalAED)}</span><br>Premium: ${C.pct(b.premium)} · Select Add transaction to save this purchase.`;}
 catch(e){el.textContent=e.message;}
}
function portfolioView(){return intro('portfolio','Investment holdings','Investment amounts, valuations and maturity details.',state.holdingDraft.id?'Edit holding':'New holding')+`<section class="panel">${head('Investment details')}<div class="grid">${field('holdingDraft.type','Product type',{options:C.types})}${field('holdingDraft.name','Product name *')}${field('holdingDraft.currency','Currency',{options:Object.fromEntries(C.currencies.map(c=>[c,c]))})}${field('holdingDraft.status','Status',{options:{proposal:'Proposal',booked:'Booked',matured:'Matured'}})}${field('holdingDraft.invested','Invested amount *',{type:'decimal'})}${field('holdingDraft.current','Current value',{type:'decimal',hint:'Leave blank when a current valuation is not available.'})}</div><details class="disclosure"><summary>Dates, income and notes</summary><div class="grid">${field('holdingDraft.start','Start date',{type:'date'})}${field('holdingDraft.maturity','Maturity date',{type:'date'})}${field('holdingDraft.yield','Yield / coupon (%)',{type:'decimal'})}${field('holdingDraft.note','Client report note',{type:'textarea'})}</div></details><div id="holdingError" class="error-box" role="alert"></div><div class="actions">${btn(state.holdingDraft.id?'Save product changes':'Add product','add-holding','primary')}${btn(state.holdingDraft.id?'Cancel edit':'Clear inputs','clear-holding')}${btn('Portfolio overview '+icon('next'),'nav','subtle','data-screen="results"')}</div></section><section class="panel">${head('Investment holdings',state.holdings.length+' recorded holdings')}<div class="record-list">${state.holdings.map(h=>`<article class="record-card"><h3 dir="auto">${E(h.name)}</h3><div class="record-meta">${E(C.types[h.type])} · ${E(statusLabel(h.status))}</div><div class="record-values"><div><small>Invested</small><strong class="num">${C.fmt(C.num(h.invested))} ${E(h.currency)}</strong></div><div><small>Current</small><strong class="num">${C.fmt(C.num(h.current))} ${E(h.currency)}</strong></div></div><div class="record-actions">${btn('Edit','edit-holding','','data-id="'+E(h.id)+'"')}${btn('Delete','delete-holding','danger','data-id="'+E(h.id)+'"')}</div></article>`).join('')||'<div class="empty">No other investments recorded. Gold is included in the portfolio overview.</div>'}</div></section>`;}
function resultsView(){
 if(isClient())return intro('results','Portfolio overview','')+R.body(state,false);
 const p=C.portfolioSummary(state),g=p.gold;
 return intro('results','Portfolio overview','Valuation, performance and consolidated holdings.')+`<section class="panel">${head('Current valuation','Valuation uses the price and date entered below, not a live quote.')}<div class="grid">${field('gold.currentAEDPerGram','Current gold price (AED/g)',{type:'decimal'})}${field('gold.currentPriceDate','Gold price date',{type:'date'})}</div><div class="actions">${btn('Update valuation','refresh-results','primary')}${btn('Exchange rates','fx')}${btn('Use today’s date','today')}</div><div id="valuationError" class="error-box" role="alert"></div></section><div class="kpis">${R.kpi('Total invested',C.money(p.invested))}${R.kpi('Current value',C.money(p.current))}${R.kpi('Performance',C.pct(p.performance),C.money(p.pnl))}${R.kpi('Weighted gold average',C.fmt(g.avgAllIn,4),'AED per gram')}${R.kpi('Gold quantity',C.fmt(g.grams/1000,6),'kilograms')}${R.kpi('Active / proposed products',String(p.products),g.valid.length+' gold batches')}</div>${p.warnings.map(w=>`<div class="helper">${E(w)}</div>`).join('')}<p class="small">Proposal and booked records are combined. Cancelled gold batches and matured products are excluded. Current value is unavailable until all active positions have a current value and a valid FX rate. Gold price date: ${E(state.gold.currentPriceDate)}.</p><div class="actions">${btn('Prepare client report','preview-client','primary')}${btn('Internal review','nav','','data-screen="internal"')}</div>`;
}
function reportsView(){return intro('reports','Reports','Prepare and review portfolio documents.')+`<section class="panel">${head('Report details')}<div class="grid">${!isClient()?field('report.type','Report type',{options:C.reportNames}):''}${!isClient()?field('report.status','Status',{options:{Draft:'Draft',Indicative:'Indicative',Approved:'Approved',Final:'Final',Booked:'Booked'}}):''}${!isClient()?field('report.date','Report date',{type:'date'}):''}</div><div class="tool-grid">${btn('Client PDF','pdf-client','primary')}${!isClient()?btn('Internal PDF','pdf-internal'):btn('Client preview','preview-client')}${btn('Client HTML snapshot','html-client')}${!isClient()?btn('Internal HTML snapshot','html-internal'):btn('Share client HTML','share-client')}</div><div class="helper">PDF uses the browser’s print dialog. The record logs preparation, not proof that a PDF was saved. Review the selected printer’s preview before sharing.</div></section>${!isClient()?`<section class="panel">${head('Editable record & backup','These files contain internal data. Keep them with the advisor, not the client.')}<div class="tool-grid">${btn('Save Master HTML','master','primary')}${btn('Link / Save same file','link-master')}${btn('Export JSON','json')}${btn('Import HTML / JSON','import')}</div><label class="check"><input id="autoMaster" type="checkbox" ${state.report.autoSaveMasterOnExport?'checked':''}> Save an updated Master HTML when preparing a report. Unsupported browsers download a new copy rather than overwrite a file.</label><p class="small">Master HTML reopens the editable workspace with this client. Client HTML is a read-only report and cannot be imported as a master.</p></section>`:''}`;}
function profileView(){return intro('client','Advisor details','Contact details and report signature.')+`<section class="panel"><div class="grid">${field('advisor.name','Advisor name')}${field('advisor.title','Title')}${field('advisor.phone','Mobile / WhatsApp',{type:'tel'})}${field('advisor.email','Email',{type:'email'})}${field('advisor.company','Company / team',{wide:true})}</div><details class="disclosure"><summary>Logo and signature images</summary><p class="small">Use a RAMY-approved logo. PNG, JPEG and WebP only, up to 2 MB. SVG is not supported for uploads.</p><label class="field"><span>Signature image</span><input id="signatureUpload" type="file" accept="image/png,image/jpeg,image/webp"></label>${C.safeImage(state.advisor.signatureImage)?`<img class="image-preview" src="${C.safeImage(state.advisor.signatureImage)}" alt="Signature preview">`:''}${btn('Remove signature image','remove-signature','subtle')}<label class="field" style="margin-top:15px"><span>RAMY logo image</span><input id="logoUpload" type="file" accept="image/png,image/jpeg,image/webp"></label>${btn('Restore supplied RAMY logo','restore-logo','subtle')}</details><div class="actions">${btn('Back to reports','nav','primary','data-screen="reports"')}</div></section>`;}
function internalView(){return intro('internal','Internal review','Confidential pricing and revenue information.')+R.body(state,true);}
function toolsBody(){
 const client=isClient();
 return `<div class="eyebrow">${E(state.client.name||'No client selected')}</div><div class="tool-grid">${btn('Client PDF','pdf-client','primary')}${client?btn('Client preview','preview-client'):btn('Internal PDF','pdf-internal')}${client?btn('Client HTML','html-client'):btn('Save Master HTML','master')}${client?btn('Share client HTML','share-client'):btn('Backup JSON','json')}</div><div class="tool-group"><h3>Appearance</h3><div class="tool-grid">${btn('Elite Beige','theme','','data-theme="beige" aria-pressed="'+(state.ui.theme==='beige')+'"')}${btn('Executive Blue','theme','','data-theme="blue" aria-pressed="'+(state.ui.theme==='blue')+'"')}</div></div>${!client?`<div class="tool-group"><h3>Workspace</h3><div class="tool-grid">${btn('Saved clients','records')}${btn('New client','new-client')}${btn('Advisor signature','nav','','data-screen="profile"')}${btn('Internal review','nav','','data-screen="internal"')}${btn('Import record','import')}${btn('Save now','save-now')}</div></div><details id="fxDetails" class="disclosure"><summary>Exchange rates · AED per currency unit</summary><div class="grid">${field('fx.USD','1 USD = AED',{type:'decimal'})}${field('fx.EUR','1 EUR = AED',{type:'decimal'})}${field('fx.GBP','1 GBP = AED',{type:'decimal'})}</div><p class="small">User-entered rates. No live rates are supplied. Imported V11 rates are preserved and must be checked before reporting.</p>${btn('Apply rates','apply-fx','primary')}</details>`:''}<div class="tool-group"><h3>Application</h3><div class="actions">${btn('Install on this phone','install')}${btn('Check for update','update')}</div><p id="installHelp" class="small" style="margin-top:12px">iPhone: open the HTTPS address in Safari, then Share → Add to Home Screen → Open as Web App. Android: use Install app from the browser menu. A downloaded HTML file is not an installed application.</p><p class="small">Storage: ${E(S.backend()||'Not available')}. ${navigator.onLine?'Network available.':'Offline.'} ${swRegistration?'Offline shell registered.':'Offline shell not registered on this page.'}</p></div><div class="helper">Saved records stay in this browser on this device. No cloud sync, server authentication or bank connection is configured. Do not distribute editable master files to clients.</div>`;
}
...
async function newClient(){
 if(!await confirmAction('Create a new client? The current client stays in saved records. Any unfinished product inputs remain with that client.','New client'))return;
...
async function saveMaster(sameFile=false){
 ...
 if(masterHandle){try{const writer=await masterHandle.createWritable();await writer.write(html);await writer.close();toast('Master file updated.');return;}catch(e){masterHandle=null;toast('Direct file access is unavailable. A new editable copy will be downloaded.');}}
...
async function dispatch(action,el){
 if(action==='gold-tab'||action==='holding-tab'){if(action==='gold-tab')goldTab=el.dataset.tab;else holdingTab=el.dataset.tab;render(screen,true);}
  ...
  if(!requireClient())return;try{const h=C.holdingFromDraft(state.holdingDraft),i=state.holdings.findIndex(x=>x.id===h.id);if(i>=0)state.holdings[i]=h;else state.holdings.push(h);audit(i>=0?'Holding edited':'Holding added',h.name);state.holdingDraft=C.blankHolding();holdingTab='records';render('portfolio',true);touch();toast(i>=0?'Holding updated.':'Holding saved for '+state.client.name+'.');}
...
function wire(){
 $('viewsToggle').onclick=()=>{const hidden=!$('modeBar').hidden;$('modeBar').hidden=hidden;$('viewsToggle').setAttribute('aria-expanded',String(!hidden));};
